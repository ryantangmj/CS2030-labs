...
class Server {
    ...
    boolean compare(Server other) {
        return this.doneTime < other.getDoneTime() ? true : false;
    }
    
    int fastestServerID(ImList<Server> servers, int serverID) {
        int fastestServer = serverID;

        for (int i = 0; i < servers.size(); i++) {
            if (!servers.get(i).isHuman() && servers.get(i).getDoneTime() <
                servers.get(fastestServer - 1).getDoneTime()) {
                fastestServer = i + 1;
            }
        }
